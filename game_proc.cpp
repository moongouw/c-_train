...
void intro() {
    cout <<
        "Введение:" << endl <<
        "Бежит мужик по лесу от лесного пожара!" << endl <<
        "Рядом собака(голодная), на поводке коза(в том же состоянии), а подмышкой капуста." << endl <<
        "Бежит и натыкается на реку, а на берегу лодка, она вмещает только двоих!" << endl <<
        "Ваша задача переправить на тот берег всех участников этого лютого забега." << endl <<
        "Все участники ведут себя хорошо только в Вашем присутствии." << endl <<
        "Как только вы оставите на едине собаку и козу, псина все непременно" << endl <<
        "сожрёт рогатую, а коза обязательно схрумкает капустку." << endl <<
        "Поехали!!!" << endl <<
        "************" << endl << endl;
}
...
void get_result(int* pos, string* name, const int LEN, int &var, bool& game, string restart, string exit) {
    ...
    for (int i = 0; i < LEN; i++)
    {
        if (pos[i] == 0)
            break;
        if (i == LEN - 1) {
            cout << "Все переправлены в целости и сохранности! Вы ВЫЙГРАЛИ!" << endl << endl << endl;
